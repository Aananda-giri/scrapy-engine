...
class BloomFilterThread(threading.Thread):
    ...
    '''
    def __init__(self, n=2000000, p= 0.00001, save_file='bloom_filter.pkl', scalable=True):
        super().__init__()
        ...
        if os.path.exists(save_file):
            ...
            if scalable:
                ...
                self.bloom = ScalableBloomFilter(mode=ScalableBloomFilter.LARGE_SET_GROWTH, initial_capacity=self.n, error_rate=self.p)
            else:
...
def get_bloom_thread(save_file='bloom_filter.pkl', n=2000000, p= 0.00001, scalable=True):
    '''
    * save bloom filter on exit
    * load bloom filter on run
    '''
    ...
    bloom_thread = BloomFilterThread(n=n, p=p, save_file=save_file, scalable=scalable)
