...
load_dotenv('.env')
assert os.environ.get('mongo_uri_v3') != None, "server/mongo.py: env. mongo_password not found"
...
class Mongo():
    def __init__(self, db_name='scrapy-engine', collection_name="urls-collection", local=False):
        if local:
            ...
            uri = os.environ.get('mongo_uri_v3')
            ...
            # one time operation
            self.collection.create_index('url', unique=True)
            # Create regular index for status
            self.collection.create_index('status') # we often perform search using status,
